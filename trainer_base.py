...
"""
このファイルは離散拡散モデル（Discrete Diffusion Model）のトレーニングフレームワークを実装しています。

主な構成要素:
1. Loss - 損失計算用のデータクラス
2. LogLinear - ノイズスケジュールを制御するモジュール
3. TrainerBase - Lightning フレームワークを使用した訓練の基本クラス

TrainerBaseクラスの特徴:
- PyTorch Lightningを使用した訓練ループの管理
- 様々なバックボーンモデル（DiT, DiMamba, HuggingFace DiT）のサポート
- EMA（Exponential Moving Average）によるモデルの安定化
- 分散学習のサポート
- チェックポイントの保存と読み込み機能
- 様々なパラメータ化（AR, Score, Mean）のサポート

このコードは離散データ（特にテキスト）に対する拡散モデルを実装しており、
ノイズ付加過程（forward process）と逆過程（reverse process）を扱うための
基盤となるクラスを提供しています。
"""
...
class TrainerBase(L.LightningModule):
    def __init__(
        ...
        # elif self.config.algo.backbone == 'dimamba': Maybe:動いていない
...
class Diffusion(TrainerBase):
    """
    Diffusion:
    - TrainerBase を継承し、拡散モデルの事前学習・サンプリングを管理するクラス
    - ノイズスケジュールの処理、損失計算、サンプリングループなどを実装
    """
    def _validate_configuration(self):
    ...
    def _sample_t(self, n, accum_step):
        """
        トレーニング時または評価時のタイムステップ t をサンプリング
        - n: バッチサイズ
        - accum_step: 勾配蓄積ステップ番号 (Noneなら評価時)
        - antithetic_sampling: 双対サンプリングを適用
        """
    ...
    def _reconstruction_loss(self, x0):
        """
        t=0 の場合の再構成損失 (分類モデルのクロスエントロピー相当)
        """
...
class UniformState(Diffusion):
    ...
    def q_xt(self, x, alpha_t):
        ...
        move_chance: float torch.Tensor with shape
            (batch_size, 1).
        """
        # 各要素ごとにノイズ追加（ランダムに置き換え）するかどうかを決定するマスクを生成
        # torch.rand は [0,1) の一様分布乱数を生成
        # 1 - alpha_t の確率で True（ノイズ置き換え）、False（元の値を保持）になる
        move_indices = torch.rand(*x.shape, device=x.device) < 1 - alpha_t

        # 語彙サイズ内でランダムトークンを生成（ノイズ用テンソル）
        uniform_tensor = torch.randint(0, self.vocab_size, x.shape, device=x.device)

        # マスクに従って、ノイズ用トークンか元の入力 x を選択
        xt = torch.where(move_indices, uniform_tensor, x)

        # BOS トークン（先頭トークン）を無視する設定の場合、必ず元の x の先頭要素を保持
        if self.ignore_bos:
            xt[:, 0] = x[:, 0]
        return xt
